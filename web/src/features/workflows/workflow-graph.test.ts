...
describe("workflow graph", () => {
    test("allocates stable typed ports and preserves connection order", () => {
        ...
        assert.deepEqual(graph.connections.map((connection) => connection.order), [0, 1]);
        assert.deepEqual(graph.nodes.find((node) => node.id === generate.id)?.inputPorts?.map((port) => port.type), ["text", "image"]);
        assert.equal(graph.connections.every((connection) => Boolean(connection.targetPortId)), true);
    });
    ...
    test("removes dependent connections and unused input ports", () => {
        ...
        assert.deepEqual(graph.nodes.map((node) => node.id), [target.id]);
        assert.deepEqual(graph.connections, []);
    ...
    test("identifies equal port ids independently for different target nodes", () => {
        ...
        const graph = { ...emptyWorkflowGraph(), nodes: [createWorkflowNode("text_input", { x: 0, y: 0 }, "source-a"), createWorkflowNode("text_input", { x: 0, y: 300 }, "source-b"), { ...createWorkflowNode("image_generation", { x: 400, y: 0 }, "target-a"), inputPorts: [{ id: "input", type: "text" as const }] }, { ...createWorkflowNode("image_generation", { x: 400, y: 300 }, "target-b"), inputPorts: [{ id: "input", type: "text" as const }] }], connections: [first, second] };
