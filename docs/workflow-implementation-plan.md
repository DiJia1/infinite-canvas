...
实施前审查确认，现有图片任务还需补齐领取令牌和旧 Worker 写回保护。作为 T4a 独立完成：提交前记录阶段；恢复时若供应商提交结果不明确且无上游任务 ID，进入待确认状态，不重新提交；已有上游 ID 则继续查询。普通画布图片任务同时识别待确认状态，停止无效轮询并给出明确提示。此项验证通过后再接入 Workflow 调度。
...
## 12. 编辑器一致性接入（2026-09-10）

Workflow 保持独立产品模块，编辑体验与普通 Canvas 属于同一编辑器家族。本次接入不改变流程执行图、稳定输出槽位、服务端调度及历史运行快照。

### 12.1 共用范围

- 画布容器、网格背景、滚轮缩放、中键／空白处空格拖动画布沿用 `InfiniteCanvas`。
- 节点选择、多选、框选、整体拖动、拖拽连线、空白落点创建和 Ctrl 切线沿用 Canvas 交互控制器；Workflow 适配器负责类型、端口、数量上限和 DAG 校验。
- 共用节点边框、选中态、连接点、缩放手柄、悬浮工具栏、配置模型选择器、参数设置面板、图片资源控制器及原生视频播放器。
- 顶部左侧文档菜单／名称／保存状态，底部中央操作栏，左下角缩放与小地图沿用共享组件。运行、停止和记录保留 Workflow 专属入口。
- 文本双击编辑，媒体原生控件与拖动隔离；图片和视频输出可独立移动，复制配置时复制其输出声明，不复制运行结果。
- 图片信息开关暂不在 Workflow 显示：当前流程节点没有普通 Canvas 的完整图片信息数据，避免提供无效果的控件。

### 12.2 保存边界

- 定义修改默认 800ms 防抖，单路串行保存并合并待保存修改；拖动／缩放节点期间暂停提交，结束后记录历史并保存。
- 本地草稿按用户及流程隔离；同版本自动恢复，远端版本已变化时由用户选择恢复草稿或使用远端，保存失败不丢弃草稿。
- 同浏览器多标签页优先用 Web Locks 保护编辑权，无该能力时复用 Canvas 的本地租约机制；接管前重新读取远端。
- 后端 revision 仍是并发写入最终保护。启动运行先保存，再传确认的 revision；版本不匹配返回 409，不创建运行。已接受的 requestId 重放仍返回原运行。
- 视角、背景和显示偏好按用户／流程保存在本地，不增加流程定义版本。撤销历史、本地偏好不混入运行快照。

### 12.3 验收与发布边界

在隔离数据库及模拟供应商环境检查创建／重开、文本编辑、选择和拖动、连线、撤销、数量选择、自动保存、标签页接管及保存后运行。共享控制器、资源生命周期及版本冲突使用针对性测试补充验证，执行 TypeScript 和生产构建。

真实供应商收费生成、上线部署和用户现有 Docker 实例重建不在本次自动执行范围内。

本轮验证记录：相关 Bun 测试 90 项通过，Workflow Go 路由测试、TypeScript 检查及生产构建通过；Chrome 隔离环境验证了文本编辑、节点拖动与撤销、框选多节点整体移动、连接配置、数量菜单、自动保存与重开、多标签只读及接管、深浅主题及 76%／100%／158% 缩放。模拟图片／视频运行均完成，视频封面可拖动，原生播放控件可以播放且不移动节点。真实供应商生成未验证。
